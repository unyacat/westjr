...
jr = westjr.WestJR(line="sanyo1", area="okayama")
...
def test_convert_stopTrains() -> None:
    ...
    駅名と停車する列車種を抽出
    """
    station = jr.get_stations(line="sanyo1").stations[0]
    assert station.info.name == "上郡"
...
def test_convert_pos() -> None:
    ...
    train = jr.get_trains(line="sanyo1").trains[0]
