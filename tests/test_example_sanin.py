...
jr = westjr.WestJR(line="sanin3", area="sanin")
...
def test_convert_stopTrains() -> None:
    ...
    駅名と停車する列車種を抽出
    """
    station = jr.get_stations(line="sanin3").stations[0]
    assert station.info.name == "諸寄"

    stopTrain_types = jr.convert_stopTrains(station.info.stopTrains)
    assert stopTrain_types == []


def test_convert_pos() -> None:
    """列車走行位置の場所を前駅と次駅の名前に変換"""
    train = jr.get_trains(line="sanin3").trains[0]
